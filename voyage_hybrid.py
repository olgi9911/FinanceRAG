"""
Example: Using LangChain's EnsembleRetriever with Voyage AI (voyage-finance-2)

This script demonstrates how to use LangChain's built-in EnsembleRetriever
combining BM25 and Voyage AI embeddings using Reciprocal Rank Fusion (RRF).

Usage:
    # Basic usage (assumes VOYAGE_API_KEY is in env)
    python voyage_langchain_hybrid.py --task FinQA

    # With explicit API key
    python voyage_langchain_hybrid.py --task FinQA --voyage_api_key "your-key-here"

Installation:
    pip install langchain langchain-community langchain-voyageai chromadb
"""
...
parser.add_argument("--persist_directory", type=str, default="./chroma_voyage", help="Directory for ChromaDB persistence")
parser.add_argument("--no_persist", action="store_true", help="Disable ChromaDB persistence (in-memory only)")
parser.add_argument("--output_dir", type=str, default="./results/hybrid_voyage", help="Directory to save results")
...
# ============================================================================
logger.info("\n[Step 2/4] Initializing Voyage AI embeddings...")

# Initialize standard LangChain Embeddings object
embeddings = VoyageAIEmbeddings(
    model="voyage-finance-2",
    # batch_size=args.batch_size,
    truncation=True # Safer for long financial docs
)

logger.info("✓ Embeddings initialized: voyage-finance-2")
